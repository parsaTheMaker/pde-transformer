...
EVT_EMA_DECAY = 0.75
TRIGGER_SCORE_EMA_DECAY = 0.75
EVT_SCORE_EPS = 1e-12
EVT_SCORE_FALLBACK = 0.05

# Thresholds are set automatically by calibrate_velocity_threshold()
VEL_EPSILON   = None   # deterministic EVT score threshold (EMA-smoothed mean score)
VEL_STD       = None   # diagnostic EVT score spread from raw epoch samples
RAW_EVT_EPS   = None   # raw epoch EVT score threshold (pre-EMA)
RAW_EVT_STD   = None   # raw epoch EVT score std
...
def get_evt_step_range():
    calib_start_step = max(1, int(MAX_ROLLOUT_LEN * 0.25))
    calib_end_step = max(calib_start_step, int(MAX_ROLLOUT_LEN * 0.75) - 1)
    return calib_start_step, calib_end_step


def positive_log_growth_stat(curr_err_per_sample, prev_err_per_sample, eps=EVT_SCORE_EPS):
    score = torch.log((curr_err_per_sample + eps) / (prev_err_per_sample + eps))
...
def calibrate_velocity_threshold(model, loader, zero_norm, get_labels_fn):
    ...
    the per-sample positive log-growth EVT score over a rollout-step range.

    EVT score at step t:
        s_t = log((E_t + eps) / (E_{t-1} + eps))

    We collect positive s_t for t in [calib_start_step, calib_end_step] across the
    first 20% of training batches, then set a deterministic threshold from the mean.

    Each epoch uses the current calibration result.
    """
    global VEL_EPSILON, VEL_STD, RAW_EVT_EPS, RAW_EVT_STD

    calib_start_step, calib_end_step = get_evt_step_range()
        ...
        for batch_idx, (x_batch, y_seq, mask) in enumerate(
            ...
            for t in range(min_steps_needed):
                ...
                if prev_err_per_sample is not None and calib_start_step <= t <= calib_end_step:
                    step_stat, _ = positive_log_growth_stat(mse_per_sample, prev_err_per_sample)
    ...
    print0(f"  Positive EVT-score batch-step candidates: {int(global_count)}")

    raw_evt_eps = None
    raw_evt_std = None
    if global_count <= 0:
        if VEL_EPSILON is None:
            VEL_EPSILON = EVT_SCORE_FALLBACK
            print0("  WARNING: no valid calibration samples found. Using EVT_SCORE_FALLBACK.")
        else:
            print0("  WARNING: no valid calibration samples found. Keeping previous smoothed EVT threshold.")
    else:
    ...
    RAW_EVT_EPS = raw_evt_eps
    RAW_EVT_STD = raw_evt_std

    raw_eps_str = f"{raw_evt_eps:.6e}" if raw_evt_eps is not None else "n/a"
    raw_std_str = f"{raw_evt_std:.6e}" if raw_evt_std is not None else "n/a"
    print0(f"  Raw EVT score eps (epoch) = {raw_eps_str}")
    print0(f"  Raw EVT score std (epoch) = {raw_std_str}")
    print0(f"  Smoothed VEL_EPSILON      = {VEL_EPSILON:.6e}")
    print0(f"  VEL_STD (raw epoch std)   = {VEL_STD:.6e}")


# ---------------------------------------------------------------------------
# Training / evaluation loop
# ---------------------------------------------------------------------------
def run_epoch(model, loader, zero_norm, get_labels_fn, training, optimizer=None, global_step=0, ema_trigger_scores=None):
    """Run one full epoch.
    ...
    Masking (obstacle enforcement):
        ...
        Error-Velocity Truncation (training only):
          - At each step t >= calib_start_step, compute the positive log-growth score:
              s_t = log((E_t + eps) / (E_{t-1} + eps))
          - Trigger condition: s_t > VEL_EPSILON
    ...
    if training:
        ...
        evt_score_threshold = max(0.0, float(VEL_EPSILON)) if VEL_EPSILON is not None else EVT_SCORE_FALLBACK
        if ema_trigger_scores is None:
            ema_trigger_scores = [None] * MAX_ROLLOUT_LEN
    else:
        evt_score_threshold = EVT_SCORE_FALLBACK
    ...
    with context():
        ...
        for step, (x_batch, y_seq, mask) in enumerate(progress_bar):
            ...
            current_evt_score_threshold = evt_score_threshold
            ...
            if training:
                states_seq = []  
                prev_err_per_sample = None
                pos_evt_score_sum = 0.0
                pos_evt_score_count = 0
                target_N = -1     
                ...
                with torch.no_grad():
                    ...
                    for t in range(max_rollout):
                        ...
                        if fixed_target == -1 and prev_err_per_sample is not None:
                            ...
                            curr_score, _ = positive_log_growth_stat(mse_per_sample, prev_err_per_sample)
                            ...
                            if curr_score is not None:
                                pos_evt_score_sum += curr_score
                                pos_evt_score_count += 1
                            else:
                                missing_pos_vel_steps += 1

                            if ema_score > current_evt_score_threshold:
                                target_N = t - 1
                                break
                ...
                # reflect the local positive-score statistic.
                if target_N >= 0:
                ...
                avg_v = pos_evt_score_sum / pos_evt_score_count if pos_evt_score_count > 0 else 0.0
    ...
    ema_summary = None
    if training and ema_trigger_scores is not None:
        active_vals = [float(v) for v in ema_trigger_scores if v is not None]
        if active_vals:
            ema_summary = {
                "active_steps": int(len(active_vals)),
    ...
    return {
        ...
        "ema_summary": ema_summary if training else None,
    }
...
def main():
    ...
    if not SKIP_TRAIN:
        if is_main_process():
            ...
            with open(loss_log_path, "w", encoding="utf-8") as log_file:
                log_file.write(
                    ...
                    "raw_evt_eps smoothed_evt_eps "
                    "train_mse train_N train_evt_score "
                    "val_mse best\n"
                )
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            # Per-rank, per-rollout-step trigger EMA state; reset every epoch.
            ema_trigger_scores = [None] * MAX_ROLLOUT_LEN
            ...
            raw_evt_eps_str = f"{RAW_EVT_EPS:.3e}" if RAW_EVT_EPS is not None else "n/a"
            smooth_evt_eps_str = f"{VEL_EPSILON:.3e}" if VEL_EPSILON is not None else "n/a"
            print0(f"\nEpoch {epoch:02d}/{EPOCHS}  LR: {current_lr:.2e}  "
                   f"raw_evt_eps={raw_evt_eps_str}  EVT_eps={smooth_evt_eps_str}  BPTT_W={BPTT_WINDOW}")
            ...
            train_loss = run_epoch(
                ...
                ema_trigger_scores=ema_trigger_scores,
            )
            ...
            print0(
                ...
                f"         avg rollout N={train_loss['N']:.2e}  max N reached={train_loss['max_N']}  max allowed N={MAX_ROLLOUT_LEN}  avg_evt_score={train_loss['vel']:.6e}\n"
                f"         trigger EMA summary: active_steps={train_loss['ema_summary']['active_steps']}  mean={train_loss['ema_summary']['mean']:.6e}  max={train_loss['ema_summary']['max']:.6e}  threshold={VEL_EPSILON:.6e}\n"
                f"         pos-score none pct={train_loss['pos_vel_none_pct']:.2f}%\n"
            ...
            if is_main_process() and loss_log_path:
                with open(loss_log_path, "a", encoding="utf-8") as log_file:
                    log_file.write(
                        ...
                        f"raw_evt_eps={raw_evt_eps_str} smoothed_evt_eps={smooth_evt_eps_str} "
                        f"train_mse={train_loss['mse']:.6e} train_N={train_loss['N']:.2e} "
                        f"train_max_N={train_loss['max_N']} "
                        f"train_evt_score={train_loss['vel']:.6e} "
                        f"train_evt_ema_mean={train_loss['ema_summary']['mean']:.6e} "
                        f"train_evt_ema_max={train_loss['ema_summary']['max']:.6e} "
