...
# Prevent cuDNN benchmark from failing to find an engine due to fragmentation
...
if torch.cuda.is_available():
    # Disable cuDNN autotuning here; the benchmark path can fail to find an engine
    # for this model/workload when memory is fragmented.
...
OUT_DIR = os.path.join("runs", "karman_mse_grad_newdir")
EPOCHS = 40
BATCH_SIZE = 36
ACCUM_GRAD = 1
LR = 4e-5
...
TASK_IMPORTANCE = {"mse": 10.0, "grad": 1.0, "direction": 1.0}
GRADNORM_LOWER_BOUND_FACTOR = 0.01
GRADNORM_UPPER_BOUND_FACTOR = 2.0
...
class GradMagAndDirectionLoss(nn.Module):
    ...
    def _erode_valid_mask(self, vm, ndim):
        if self.erosion_radius <= 0:
            return vm

        kernel = 2 * self.erosion_radius + 1
        padding = self.erosion_radius
        inv_vm = 1.0 - vm
        if ndim == 4:
            inv_vm = F.max_pool2d(inv_vm, kernel_size=kernel, stride=1, padding=padding)
        elif ndim == 5:
            inv_vm = F.max_pool3d(inv_vm, kernel_size=kernel, stride=1, padding=padding)
        else:
            raise ValueError(f"Unsupported ndim for erosion: {ndim}")
        return 1.0 - inv_vm
    ...
    def _prepare_mask(self, valid_mask, x):
        ...
        # Erode the valid region by at least one stencil radius so boundary-adjacent
        # fluid cells do not sample obstacle interior values in central differences.
        vm = self._erode_valid_mask(vm, x.ndim)
            ...
            if training:
                ...
                if do_gradnorm:
                    ...
                    for i in range(len(loss_vec)):
                        grad_w = torch.autograd.grad(loss_vec[i], last_layer_weight, retain_graph=(i < len(loss_vec) - 1))[0]
                    ...
                    with torch.no_grad():
                        expected_sum = sum(TASK_IMPORTANCE[n] for n in ACTIVE_LOSSES)
                        
                        # Iterative projection to satisfy both bounds AND sum constraint
                        for _ in range(15):
                            normalize_coeff = expected_sum / lambda_weights.sum()
                            lambda_weights.data = lambda_weights.data * normalize_coeff
                            
                            clamped = False
                            for i, name in enumerate(ACTIVE_LOSSES):
                                base_val = TASK_IMPORTANCE[name]
                                new_val = torch.clamp(lambda_weights.data[i], base_val * GRADNORM_LOWER_BOUND_FACTOR, base_val * GRADNORM_UPPER_BOUND_FACTOR)
                                if new_val != lambda_weights.data[i]:
                                    clamped = True
                                lambda_weights.data[i] = new_val
...
def main():
    ...
    loss_fn = GradMagAndDirectionLoss(
        ...
        erosion_radius=LOSS_MASK_EROSION_RADIUS,
    ).to(DEVICE)
    ...
    print(f"  Valid-mask erosion radius: {loss_fn.erosion_radius}")
    print(
        "  Combo weights: "
    ...
    if num_active_tasks > 1:
        init_weights = [TASK_IMPORTANCE[n] for n in ACTIVE_LOSSES]
        lambda_weights = nn.Parameter(torch.tensor(init_weights, dtype=torch.float32, device=DEVICE))
