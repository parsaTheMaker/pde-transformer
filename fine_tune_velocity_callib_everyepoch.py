"""
fine_tune_velocity_callib_everyepoch.py
=====================
...
Key design decisions:
  ...
  - EVT trigger: only fire if the step-wise velocity (E_t - E_{t-1})
    exceeds the calibrated threshold; backprop through step t-1.
    - Tolerance (VEL_EPSILON) and per-sample noise std (VEL_STD) are calibrated
        automatically before every epoch from a no-grad survey pass.
...
OUT_DIR = os.path.join("runs", "karman_finetuned_velocity_LoRA_callib_everyepoch")
EPOCHS = 40
BATCH_SIZE = 24
ACCUM_GRAD = 1
LR = 4e-5
...
def calibrate_velocity_threshold(model, loader, zero_norm, get_labels_fn):
    ...
    the *relative* first-differences of MSE errors at the target step.

    Velocity at step t:
        v_t = E_t - E_{t-1}

    We collect v_t at CALIB_TARGET_STEP across the first 5% of training
    batches, then set:
        VEL_EPSILON   = mean(v_t values)
        VEL_STD       = std(v_t values)
    """
    global VEL_EPSILON, VEL_STD

    min_steps_needed = CALIB_TARGET_STEP + 1
    if min_steps_needed < 2:
        raise ValueError("CALIB_TARGET_STEP must be >= 1 so that a first-difference exists.")

    print0(f"\nCalibrating EVT thresholds from train set (target step {CALIB_TARGET_STEP + 1}) ...")
    print0(f"  Using first 5% of train batches for speed.")
    model.eval()
    v_values = []
    max_calib_batches = max(1, len(loader) // 20)
    ...
    if global_count <= 0:
        ...
        VEL_EPSILON = fallback_v_eps
        VEL_STD = 0.5 * fallback_v_eps
    else:
        VEL_EPSILON = float(global_sum / global_count)
        var = max((global_sum_sq / global_count) - (VEL_EPSILON ** 2), 0.0)
        VEL_STD = float(np.sqrt(var))
        if VEL_EPSILON <= 0:
            ...
            VEL_EPSILON = max(float(floor_t.item()), 1e-8)
...
def run_epoch(model, loader, zero_norm, get_labels_fn, training, optimizer=None, global_step=0):
    ...
    Error-Velocity Truncation (training only):
        ...
        - When triggered: backprop through the previous step (step t-1).
    """
    if training:
        model.train()
    ...
    if training and VEL_EPSILON is not None and VEL_STD is not None:
        _vel_thresholds = np.maximum(
            0.0, np.random.normal(loc=VEL_EPSILON, scale=VEL_STD, size=n_batches)
        ).tolist()
    else:
        ...
        for step, (x_batch, y_seq, mask) in enumerate(progress_bar):
            ...
            if training:
                ...
                if target_N >= 0:
                    ...
                    n_e = len(errors)
                    if n_e >= 2:
                        abs_vels = [errors[i] - errors[i-1] for i in range(1, n_e)]
                        avg_v = sum(abs_vels) / len(abs_vels) if abs_vels else 0.0
                    else:
                        avg_v = 0.0
    ...
    if not SKIP_TRAIN:
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            print0(
                ...
                f"         avg rollout N={train_loss['N']:.2e}  max={MAX_ROLLOUT_LEN}  avg_vel={train_loss['vel']:.6e}\n"
