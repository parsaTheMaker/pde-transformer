"""
fine_tune_velocity_bptt_calib_everyepoch2.py
=====================
...
OUT_DIR = os.path.join("runs", "karman_finetuned_velocity_LoRA_bptt_calib_everyepoch2")
EPOCHS = 40
BATCH_SIZE = 12
ACCUM_GRAD = 1
LR = 5e-5
    ...
    with torch.inference_mode():
        for batch_idx, (x_batch, y_seq, mask) in enumerate(
            ...
            if len(errors) == min_steps_needed:
                for t in range(calib_start_step, calib_end_step + 1):
                    v_values.append(errors[t] - errors[t - 1])
