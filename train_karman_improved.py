...
def run_epoch(model, loader, zero_norm, get_labels_fn, loss_fn, training, optimizer=None, operator=None, global_step=0):
   ...
   with context():
       ...
       for step, (x, y, mask) in enumerate(progress_bar):
           ...
           y_masked = torch.lerp(zero_norm, y, mask)
           ...
           if training:
               ...
               if active_loss_name == "mse":
                   mse_loss = F.mse_loss(pred, y_masked)
                   loss_to_backprop = mse_loss * LOSS_WEIGHT_MSE
                   with torch.no_grad():
                       grad_loss, dir_loss = loss_fn.get_components(pred.detach(), y_masked, valid_mask=mask)
               elif active_loss_name == "grad":
                   grad_loss, dir_loss = loss_fn.get_components(pred, y_masked, valid_mask=mask)
