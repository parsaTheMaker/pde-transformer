...
# Loss configuration
# This version only uses MSE loss, mask parameters and lambda_weights removed.
...
def update_progress(progress_bar, loss_dict, sample_count):
    ...
    mse_avg = loss_dict["mse"] / denom
    if isinstance(mse_avg, torch.Tensor):
        mse_avg = mse_avg.detach().item()
    progress_bar.set_postfix(mse=f"{mse_avg:.6f}")
...
def run_epoch(model, loader, get_labels_fn, training, optimizer=None):
    if training:
        model.train()
        if optimizer:
    ...
    loss_accum = {"mse": torch.zeros((), device=DEVICE)}
    sample_count = 0
    ...
    return {"mse": (loss_accum["mse"] / max(1, sample_count)).detach().item()}
...
def main():
    ...
    if not SKIP_TRAIN:
        ...
        with open(loss_log_path, "w", encoding="utf-8") as log_file:
            log_file.write("timestamp epoch lr train_mse val_mse best\n")
